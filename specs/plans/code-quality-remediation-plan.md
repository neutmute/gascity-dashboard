...
Status: implementation in progress. WS-1 through WS-9, WS-12, and WS-14 are complete in the current branch; WS-10 is partially complete; WS-11 and the independent WS-13 items remain planned. This plan is the synthesis of two independent review passes against `main` (clean working tree, ~31k LOC of non-test source):
...
- **Implementation status (2026-05-31):** Complete for the shared barrel split and list-generic work. `shared/src/index.ts` is now a thin 30-line package-root barrel. Runtime values Claude called out live in leaves and remain value-exported from the barrel: `operator.ts` owns `OPERATOR_DISPLAY_ALIAS`, `OPERATOR_WIRE_ALIAS`, `GC_EVENT_PREFIX`, and `errorMessage`; `context-window.ts` owns `TRUE_CONTEXT_WINDOWS` and `effectiveContextPct`. `lists.ts` owns `Avail<T>`, `GcPartialAware`, `GcList<T>`, `GcCountedList<T>`, and `GcRequiredPartialList<T>`, with repeated list envelopes wired through those generics. Domain DTOs moved to focused leaves: `transcript.ts`, `gc-agents.ts`, `gc-rigs.ts`, `gc-beads.ts`, `gc-mail.ts`, `activity.ts`, `gc-health.ts`, `gc-events.ts`, `formula-runs.ts`, `api-error.ts`, and `maintainer-triage.ts`. Remaining supervisor-wire shedding belongs to WS-10 G-2/G-3, where the generated OpenAPI client and accurate upstream schemas replace temporary dashboard-side adapters.
...
- **Implementation status (2026-05-31):** In progress. G-0 is complete: repo engines and CI now require Node `>=22.13.0`. G-1a is complete: `backend/openapi-ts.config.ts` generates a committed hey-api SDK/type/client folder under `backend/src/generated/gc-supervisor-client`, and `openapi:gc-supervisor:check` compares only that generated tree. G-1b transport cutover is complete: `GcClient` now calls the generated SDK, and the old `openapi-typescript` artifacts, `openapi-fetch` dependency, custom schema-map extractor, AJV overlay, and generated schema validator have been deleted. The remaining temporary code at this boundary is the hand-Zod dashboard DTO adapter (`gc-supervisor-decoders.ts`), which uses generated hey-api response types and exists only until G-2/G-3 can turn on generated-Zod response validation without rejecting currently-valid degraded supervisor payloads.
...
- **Generated code is backend-only.** The security model (backend binds 127.0.0.1, redacts, proxies; the browser only talks to `/api/*`) keeps supervisor types out of the frontend bundle. The generator inputs should be the supervisor OpenAPI document plus `backend/openapi-ts.config.ts`; `scripts/generate-gc-supervisor-client.mjs` should be a thin regenerate/check wrapper, not a second schema generator. Output under `backend/src/generated/gc-supervisor-client/`.
...
- **Phased rollout (forced by the accuracy dependency):**
  ...
  - **G-1b (hard cutover, no compatibility aliases):** Re-point `GcClient` internals at the generated SDK (transport + types) with response validation **off / lenient (`passthrough`)** only where upstream schema accuracy is not ready. Delete the hand request/path/operation plumbing, `openapi-fetch` client code, old generated `openapi-typescript` artifacts, and custom schema extractor. Translate `workflow_id → run_id` in a thin dashboard DTO adapter; do not expose old dashboard aliases.
  ...
  - **G-3 (strict generated validation):** Enable generated-**Zod response validation** at the `GcClient` edge; delete any remaining temporary normalizers that only existed to bridge schema drift. A malformed payload now fails at the edge per the spec invariant. This subsumes the WS-13 `getStatus`/`decodeSling` all-optional fixes (the generated validators + accurate required fields replace those hand schemas). The only surviving hand code at this boundary should be dashboard policy and explicit DTO mapping the generator cannot express.
- **hey-api features to leverage (verified against current docs, May 2026 — maximize generated code per the directive):**
  ...
  - **Prerequisite (verified):** hey-api is **ESM-only as of 2026** and requires Node `>=22.13.0`; backend is already `"type": "module"` + `moduleResolution: bundler`. The migration removes `ajv` (after G-3), supersedes `openapi-typescript`, and removes `openapi-fetch` once generated transport lands.
...
Tier 3 (decomposition)   WS-10 G-0 (Node 22/tooling) ─► WS-10 G-1a (generate @hey-api)
                         ─► WS-10 G-1b (hard cutover/delete old stack) ─► [gascity OpenAPI accuracy = G-2] ─► WS-10 G-3 (strict Zod)
                         WS-9 (after WS-2 + WS-10 G-1b), WS-11 (after WS-3), WS-14 (complete)
Tier 4 (correctness)     WS-12 (complete), WS-13 (getStatus/decodeSling fold into WS-10 G-3)
```

**Recommended order:** WS-1, WS-2 → WS-3, WS-4 (quick wins, reverse drift) → WS-5, WS-6, WS-7 (canonical resolvers/policy) → **WS-10 G-0/G-1a/G-1b** (Node 22/tooling, generate with @hey-api, hard-cut to the generated SDK, delete the old client stack) + WS-13 cheap-correctness items → WS-8, WS-9, WS-12 and WS-14 (complete), WS-11 (decomposition) → **[upstream gascity accuracy = WS-10 G-2]** (start early; it gates G-3) → **WS-10 G-3** (strict generated-Zod validation). Kick off the cross-repo G-2 work as soon as G-1b lands, since it's the long pole.
...
Root `npm run typecheck` already includes backend and frontend test typechecks. `npm run build:shared` is listed separately to mirror CI setup order even though `typecheck:src` also builds shared. For WS-10 generator work, also run `npm run openapi:gc-supervisor:generate` before the check and commit generated artifacts.
...
- **WS-9 split the shared barrel without adding subpath API churn.** Remaining raw supervisor-wire mirror removal is now a WS-10 G-2/G-3 concern: accurate upstream OpenAPI + generated response validation should let `shared` keep only dashboard-owned run-vocab DTOs.
- **The architecture spec has been amended** (`specs/architecture/formula-run-detail-type.md`): *Naming Boundary*, *UI Consumption*, *Ideal Target State*, *Invariants*, *Risk #5*, and *Current Implementation Against The Ideal* now describe the generated `@hey-api` client + runtime validation, the two-resource hook model, and tab-as-user-state. `AGENTS.md` and `specs/architecture/overview.md` now match the generated-supervisor/dashboard-DTO boundary; keep `specs/requirements/modular-dashboard-prd.md`, `specs/architecture/module-author-checklist.md`, and `PRODUCT.md` under review as the remaining durable product docs are revised around `/runs` vocabulary and module boundaries.

### Remaining risk to watch

- **The Node 22/tooling move (WS-10 G-0)** is a hard prerequisite for current `@hey-api/openapi-ts`.
- **The cross-repo accuracy dependency (WS-10 G-2)** is the long pole: strict generated-Zod validation (G-3) cannot ship until gascity's OpenAPI matches observed supervisor output, or the 15 previously-degraded payload shapes will be rejected. Start the upstream work as soon as G-1b lands.
